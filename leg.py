...
class Puzzle():
    ...
    def SavePuzzle(self):
        filename = str(input("Enter filename to be saved"))
        with open(filename+".txt", "w") as f:
            f.write(str(len(self.__AllowedSymbols))+"\n")
            for Count in range(1, len(self.__AllowedSymbols)+1):
                f.write(self.__AllowedSymbols[Count-1]+"\n")
            f.write(str(len(self.__AllowedSymbols))+"\n")    
            print(self.__AllowedPatterns) 
            for Count in range(1, len(self.__AllowedPatterns)+1): 
                f.write(self.__AllowedPatterns[Count-1].GetPatternSequence()+"\n") 
            f.write(str(len(self.__AllowedPatterns))+"\n")  
            f.write(str(self.__GridSize)+"\n")
            for Count in range(1, self.__GridSize * self.__GridSize + 1): 
                if self.__Grid[Count-1].GetSymbol() != "-": 
                    f.write(self.__Grid[Count-1].GetSymbol()) 
                else: 
                    f.write(",") 
                for CurrentSymbol in range(0, len(self.__Grid[Count-1]._Cell__SymbolsNotAllowed)):
                    f.write(","+self.__Grid[Count-1]._Cell__SymbolsNotAllowed[CurrentSymbol])
                f.write("\n")
    ...
    def AttemptPuzzle(self):
        ...
        while not Finished:
            ...
            while not Valid:
                try:
                    Row = input("Enter row number/ Save (S): ").upper() 
                    if Row == "S":
                        self.SavePuzzle() 
                    Valid = True
                except:
            ...
            while not Valid:
                try:
                    Column = input("Enter column number/ Save (S): ").upper() 
                    if Column ==  "S":
                        self.SavePuzzle() 
                    Valid = True
                except:
